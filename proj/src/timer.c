...
int (timer_set_frequency)(uint8_t timer, uint32_t freq) {
  ...
  uint32_t timer_counter = TIMER_FREQ / freq;
  uint8_t lsb, msb;
  ...
  if(sys_outb(timer_counter, lsb) != 0) return 1;
  if(sys_outb(timer_counter, msb) != 0) return 1;
...
int (timer_subscribe_int)(uint8_t *bit_no) {
  ...
  *bit_no = BIT(timer_hook_id);
  int sys_policy = sys_irqsetpolicy(TIMER0_IRQ, IRQ_REENABLE, &timer_hook_id);
